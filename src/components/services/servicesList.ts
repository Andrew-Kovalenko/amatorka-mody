import { IService, ServiceType } from './types';

export const servicesList: IService[] = [
    {
        type: ServiceType.LINES,
        title: 'Розбір ліній зовнішності',
        details: {
            price: '70 доларів',
            format: 'online',
            ETA: '7-10 робочих днів з моменту надсилання фото та заповнення анкети.',
            lists: [
                {
                    title: 'Ця послуга потрібна вам, якщо:',
                    items: [
                        {text: 'який у вас тип фігури, підтон шкіри, контрастність;',},
                        {text: 'які фасони, силуети, фактури, форми та довжини обирати, щоб підкреслити свою зовнішність;',},
                        {text: 'як працювати з кольорами та їх насиченістю в образах;',},
                        {text: 'як обирати прикраси та аксесуари.',},
                    ]
                },
                {
                    title: 'Схема роботи:',
                    items: [
                        {text: 'Короткий зідзвон для знайомства та формування індивідуального запиту.',},
                        {text: 'Заповнення анкети (надсилання фото портретної зони і фігури). Пам’ятка як правильно зробити фото додається.',},
                        {text: 'Зідзвон для представлення результатів роботи.',},
                    ]
                },
                {
                    title: 'Що отримаєте:',
                    items: [
                        {text: 'презентацію, де буде продемонстровано: розбір ваших ліній  зовнішності, рекомендації щодо гармонійних кольорів, фасонів, силуетів, фактур, форм та довжин; формули образів, відповідно до вашого запиту та ліній зовнішності;',},
                        {text: 'запис фінальної зустрічі, яку зможете у будь-який момент передивитись та переслухати;',},
                        {text: 'мою підтримку протягом двох тижнів після консультації. Зможете задати будь-які питання, що виникнуть після наданої інформації.',},
                    ],
                },
            ],
        },
    },
    {
        type: ServiceType.WARDROBE_ONLINE,
        title: 'Online Розбір гардеробу + шопінг-супровід',
        notes: 'У випадку наявності великої кількості позицій для розбору, ціна коригується.',
        details: {
            price: '150 доларів',
            format: 'online',
            ETA: ' 7-10 робочих днів з моменту надсилання фото та заповнення анкети.',
            lists: [
                {
                    title: 'Ця послуга потрібна вам, якщо:',
                    items: [
                        {text: 'речей багато, але все набридло/не подобається і вдягти «нічого»'},
                        {text: 'маєте сумніви щодо актуальності або доречності більшості речей '},
                        {text: 'не знаєте, як складати комплекти з наявних позицій'},
                        {text: 'хочете розібратися одяг та аксесуари яких силуетів, фасонів, форм та кольорів купувати у майбутньому, щоб вони «працювали» у вашому гардеробі.'},
                    ],
                },
                {
                    title: 'Схема роботи:',
                    items: [
                        {text: 'Короткий зідзвон для знайомства та формування індивідуального запиту.'},
                        {text: 'Заповнення анкети (у тому числі надсилання фото портретної зони і фігури). Пам’ятка як правильно зробити фото додається.'},
                        {text: 'Надсилання фото позицій з вашого гардеробу (розбираємо по сезонам: зима/осінь або весна/літо). Пам’ятка як правильно зробити фото додається.'},
                        {text: 'Зідзвон для представлення результатів роботи.'},
                    ],
                },
                {
                    title: 'Що отримаєте:',
                    items: [
                        {
                            text: 'презентацію, де буде продемонстровано:',
                            innerList: [
                                'розбір ваших ліній зовнішності;',
                                'розбір гардеробу з аргументаціями;',
                                'шопінг-лист, відповідно до запиту і бюджету з конкретними позиціями;',
                                'сформовано декілька капсул на сезон, що розбираємо;',
                            ],
                        },
                        {text: 'запис фінальної зустрічі, яку ви зможете у будь-який момент передивитись та переслухати;'},
                        {text: 'мою підтримку протягом двох тижнів після консультації. Зможете задати будь-які питання, що виникають.'},
                    ],
                },
            ],
            listSummary: 'Тобто, ви отримуєте дві послуги - розбір гардеробу та шопінг-супровід. Саме такий підхід,' +
                ' на мою думку, є органічним та ефективним.'
        },
    },
    {
        type: ServiceType.WARDROBE_OFFLINE,
        title: 'Offline Розбір гардеробу + шопінг-супровід',
        notes: '3/4 години. Кожна наступна година додатково оплачується.',
        details: {
            price: '150 доларів',
            format: 'offline',
            ETA: ' 7-10 робочих днів з моменту надсилання фото та заповнення анкети.',
            lists: [
                {
                    title: 'Ця послуга потрібна вам, якщо:',
                    items: [
                        {text: 'речей багато, але все набридло/не подобається і вдягти «нічого»'},
                        {text: 'маєте сумніви щодо актуальності або доречності більшості речей'},
                        {text: 'не знаєте, як складати комплекти з наявних позицій'},
                        {text: 'хочете розібратися одяг та аксесуари яких силуетів, фасонів, форм та кольорів купувати у майбутньому, щоб вони «працювали» у вашому гардеробі.'},
                    ],
                },
                {
                    title: 'Схема роботи:',
                    items: [
                        {text: 'Короткий зідзвон для знайомства та формування індивідуального запиту. Визначення часу, дати для зустрічі.'},
                        {text: 'Заповнення анкети (у тому числі надсилання фото портретної зони і фігури). Пам’ятка як правильно зробити фото додається.'},
                        {text: 'Особиста зустріч та безпосереднє проведення консультації (+ поради по організації гардеробного простору). Складання та оцифрування гардеробу та окремих образів.'},
                    ],
                },
                {
                    title: 'Що отримаєте:',
                    items: [
                        {text: 'презентацію, де буде продемонстровано: 1) розбір ваших ліній зовнішності; 2) розбір гардеробу з аргументаціями; 3) шопінг-лист, відповідно до запиту і бюджету з конкретними позиціями; 4) сформовано декілька капсул на сезон, що розбираємо;'},
                        {text: 'запис фінальної зустрічі, яку ви зможете у будь-який момент передивитись та переслухати;'},
                        {text: 'мою підтримку протягом двох тижнів після консультації. Зможете задати будь-які питання, що виникають.'},
                    ],
                },
            ],
            listSummary: 'Тобто, ви отримуєте дві послуги - розбір гардеробу та шопінг-супровід. Саме такий підхід,' +
                ' на мою думку, є органічним та ефективним.',
        },
    },
    {
        type: ServiceType.CUPSULA,
        title: 'Формування капсули',
        details: {
            price: '150 доларів',
            format: 'online',
            ETA: ' 7-10 робочих днів з моменту надсилання фото та заповнення анкети.',
            lists: [
                {
                    title: 'Ця послуга потрібна вам, якщо:',
                    items: [
                        {text: 'ви прихильник невеликого гардеробу, де всі речі ідеально поєднуються між собою;'},
                        {text: 'цінуєте власний час або/та не любите довго думати, що вдягти;'},
                        {text: 'бажаєте переписати свою гардеробну історію з чистого аркушу, і оновити більшість позицій;'},
                        {text: 'хочете скласти комплекти для певних життєвих обставин - поїздка/на сезон/в офіс тощо.'},
                    ]
                },
                {
                    title: 'Схема роботи:',
                    items: [
                        {text: 'Короткий зідзвон для знайомства та формування індивідуального запиту.',},
                        {text: 'Заповнення анкети (та надсилання фото портретної зони і фігури). Пам’ятка як правильно зробити фото додається.',},
                        {text: 'Надсилання фото позицій з вашого гардеробу (відповідно до запиту). Пам’ятка як правильно зробити фото додається.',},
                        {text: 'Зідзвон для представлення результатів роботи.',},
                    ]
                },
                {
                    title: 'Що отримаєте:',
                    items: [
                        {text: 'презентацію, де буде продемонстровано: розбір ваших ліній  зовнішності, складену капсулу (20-40 готових комплектів) та шопінг-лист, відповідно до запиту і бюджету з конкретними позиціями;',},
                        {text: 'запис фінальної зустрічі, яку ви зможете у будь-який момент передивитись та переслухати;',},
                        {text: 'мою підтримку протягом двох тижнів після консультації. Зможете задати будь-які питання, що виникають. Наприклад, скидати фото речей, які ви придбали з шопінг-листа (або зробили вибір речі самостійно), задля отримання поради щодо розміру, посадки тощо.',},
                    ],
                },
            ]
        },
    },
    {
        type: ServiceType.WARDROBE_STYLING,
        title: 'Стайлінг гардеробу',
        details: {
            price: '50-100 доларів (в залежності від кількості речей)',
            format: 'online',
            ETA: ' 7-10 робочих днів з моменту надсилання фото.',
            lists: [
                {
                    title: 'Ця послуга потрібна вам, якщо:',
                    items: [
                        {text: 'ви вже були моїм клієнтом, і замовляли послугу розбір гардеробу або/та формування капсули;'},
                        {text: 'ви придбали одну чи декілька позицій, і бажаєте максимально вдало та функціонально вписати її в свій гардероб.'},
                    ]
                },
                {
                    title: 'Схема роботи:',
                    items: [
                        {text: 'Короткий зідзвон для формування індивідуального запиту.'},
                        {text: 'Надсилання фото придбаних позицій. Пам’ятка як правильно зробити фото додається.'},
                        {text: 'Зідзвон для представлення результатів роботи.'},
                    ]
                },
                {
                    title: 'Що отримаєте:',
                    items: [
                        {text: 'презентацію, де буде продемонстровано: складені комплекти на відповідний сезон за запитом;',},
                        {text: 'запис фінальної зустрічі, яку ви зможете у будь-який момент передивитись та переслухати;',},
                        {text: 'мою підтримку протягом двох тижнів після консультації. Зможете задати будь-які питання, що виникають.',},
                    ],
                },
            ]
        },
    },
    {
        type: ServiceType.STYLIST_FOR_YEAR,
        title: 'Стиліст на рік',
        details: {
            price: '500 доларів',
            format: 'online/offline',
            ETA: '7-10 робочих днів для кожної послуги або за запитом',
            lists: [
                {
                    title: 'Ця послуга потрібна вам, якщо:',
                    items: [
                        {text: 'бажаєте повністю делегувати роботу зі стилем;'},
                        {text: 'хочете глибинно пропрацювати свій гардероб та створити стильовий вектор на найближчі роки.'},
                    ],
                },
                {
                    title: 'Що входить у дану послугу:',
                    items: [
                        {text: 'розбір ліній зовнішності;'},
                        {text: '2 розбори гардеробу + шопінг-супроводи (на сезон весна/літо та осінь/зима відповідно)'},
                        {text: '1 капсула за будь-яким запитом;'},
                        {text: '2 послуги стайлінгу гардеробу;'},
                    ],
                },
                {
                    title: 'Що отримаєте:',
                    items: [
                        {text: 'презентацію, де буде продемонстровано: ',
                            innerList: [
                                'розбір ваших ліній зовнішності;',
                                'розбір гардеробу з аргументаціями;',
                                'шопінг-лист, відповідно до запиту і бюджету з конкретними позиціями;',
                                'сформовано декілька капсул на сезон, що розбираємо;',
                            ]
                        },
                        {text: 'запис фінальної зустрічі, яку ви зможете у будь-який момент передивитись та переслухати;'},
                        {text: 'мою підтримку протягом двох тижнів після консультації. Зможете задати будь-які питання, що виникають.'},
                    ],
                },
            ],
            listSummary: '*Опис зазначених послуг читайте вище',
        },
    },
]

